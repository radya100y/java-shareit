...
public class BookingIntegrityTest {
    ...
    void shouldSave() {
        ...
        UserDto user1 = UserDto.builder().email("qwe@qwe.qwe").name("qwe").build();
        UserDto user2 = UserDto.builder().email("wer@wer.wer").name("wer").build();
        ItemDto item1 = ItemDto.builder().name("qwe").description("qwe").available(true).owner(1L).build();
        BookingDto booking = BookingDto.builder().itemId(1L)
                .start(LocalDateTime.now().plus(10, ChronoUnit.HOURS))
                .end(LocalDateTime.now().plus(20, ChronoUnit.HOURS))
                .userId(2L).status(BookingStatus.WAITING).build();

        userService.save(user1);
        userService.save(user2);
        itemService.save(item1, 1L);
        ...
        Assertions.assertEquals(bookings.get(0).getBooker().getId(), 2L);
    }
}
